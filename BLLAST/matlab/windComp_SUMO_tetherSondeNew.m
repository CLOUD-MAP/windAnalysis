...
for iFlight = 1: nFlights
    ...
    if ~strcmp(sumoData(iFlight).a_type, 'profile')
        ...
        elseif 10*length(sumoData(iFlight).WSS) + 1 > length(sumoData(iFlight).time)
            ...
            % -------------------------------------
            sumo.meanLat_deg = nanmean(sumoData(iFlight).lat_asc);
            sumo.meanLon_deg = nanmean(sumoData(iFlight).lon_asc);
            fprintf('Lat, Lon = %7.4f N, %7.4f E\n', ...
                sumo.meanLat_deg, sumo.meanLon_deg)
            ...
            datetick('x', 13)
            shg
            plotName = sprintf('tether_%4.4d%2.2d%2.2d_%2.2d_1.png', ...
                procYear, procMonth, procDay, iFlight);
            print([outDir plotName], '-dpng')
            % -------
            figure(2)
            % -------
            clf
            ...
            zlabel('Height (m)')
            title([datestr(begTime) ' - ' datestr(endTime)], 'fontsize', fontSize)
            shg
            plotName = sprintf('tether_%4.4d%2.2d%2.2d_%2.2d_2.png', ...
                procYear, procMonth, procDay, iFlight);
            print([outDir plotName], '-dpng')
            % -------
            figure(3)
            ...
            xlabel('v (m/s)')
            ylabel('Height (m)')
            suptitle([datestr(begTime) ' - ' datestr(endTime)], 'fontsize', fontSize)
            shg
            plotName = sprintf('tether_%4.4d%2.2d%2.2d_%2.2d_3.png', ...
                procYear, procMonth, procDay, iFlight);
            print([outDir plotName], '-dpng')
            % -------
            figure(4)
            ...
            suptitle([datestr(begTime) ' - ' datestr(endTime)], 'fontsize', fontSize)
            shg
            plotName = sprintf('tether_%4.4d%2.2d%2.2d_%2.2d_4.png', ...
                procYear, procMonth, procDay, iFlight);
            print([outDir plotName], '-dpng')
            % -------
            figure(5)
            ...
            suptitle([datestr(begTime) ' - ' datestr(endTime)], 'fontsize', fontSize)
            shg
            plotName = sprintf('tether_%4.4d%2.2d%2.2d_%2.2d_5.png', ...
                procYear, procMonth, procDay, iFlight);
            print([outDir plotName], '-dpng')
            % -------
            figure(42)
            ...
            clf
            subplot(1, 2, 1)
            plot(sumoData(iFlight).WSS, sumoData(iFlight).W_alt, '-*b')
            set(gca, 'xlim', [0 15])
            set(gca, 'ylim', heightLim)
            set(gca, 'fontsize', fontSize)
            set(gca, 'linewidth', lineWidth)
            ...
            ylabel('Height (m)')
            subplot(1, 2, 2)
            plot(sumoData(iFlight).WDD, sumoData(iFlight).W_alt, '-*b')
            set(gca, 'xlim', [0 360])
            set(gca, 'ylim', heightLim)
            set(gca, 'fontsize', fontSize)
            set(gca, 'linewidth', lineWidth)
            ...
            suptitle([datestr(begTime) ' - ' datestr(endTime)], 'fontsize', fontSize)
            shg
            plotName = sprintf('tether_%4.4d%2.2d%2.2d_%2.2d_42.png', ...
                procYear, procMonth, procDay, iFlight);
            print([outDir plotName], '-dpng')
            % Option to quit
